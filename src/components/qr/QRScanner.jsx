...
export default function QRScanner({ prestataireId, onScanned, onClose }) {
  ...
  const processQR = async (data) => {
    ...
    try {
      ...
      // Vérifier le plafond journalier avant l'insert
      let ptsAAttribuer = 0
      let plafondAtteint = false
      let maxJour = parseInt(window.SIOK_PARAMS?.points_max_jour) || 5
      if (qr.etudiantId && qr.etudiantId !== 'anon') {
        ...
        plafondAtteint = pointsJour >= maxJour
        ptsAAttribuer = plafondAtteint ? 0 : (parseInt(window.SIOK_PARAMS?.points_qrc) || 3)
      }
      ...
      if (qr.etudiantId && qr.etudiantId !== 'anon' && !plafondAtteint) {
        try {
          ...
          if (et) {
            await db().from('etudiants').update({ points: (et.points || 0) + ptsAAttribuer }).eq('id', qr.etudiantId)
          }
